...
$checks = [ordered]@{
    ...
    '실제 EXE SourceRoot 분리' = -not $bootstrap.Contains("'-SourceRoot'")
    'WorkerScript 절대 경로 정규화' = $bootstrap.Contains('[IO.Path]::GetFullPath($WorkerScript.Trim().Trim(')
    '상세 예외 로그' = $bootstrap.Contains('Exception.GetType().FullName') -and $bootstrap.Contains('InvocationInfo.ScriptLineNumber') -and $bootstrap.Contains('ScriptStackTrace') -and $installer.Contains('Write-DetailedInstallException')
    '루트 직하위 설치 경로 처리' = $installer.Contains('[IO.Path]::GetDirectoryName($script:InstallRoot)') -and $installer.Contains('[IO.Directory]::CreateDirectory($installParent)')
